...
pilko = toTuple . words
	where
		toTuple [f,s] = Just (f,s)
		toTuple _ = Nothing

tarkastaNumero (f,s)
	| any (`elem` ['0'..'9']) $ f ++ s = Nothing
	| otherwise = Just (f,s)

tarkastaIsotAlkukirjaimet ret@(f':f,s':s)
	| isLower f' || isLower s' = Nothing
	| otherwise = Just ret
...
chainList [] = Just []
chainList (m:ms) = m ?> (\m' -> chainList ms ?> (\ms' -> Just $ m':ms'))
...
sumPos xs = chainList (mapPos xs) ?> (\xs' -> Just $ sum xs')
	where mapPos = map (\x -> if x >= 0 then Just x else Nothing)
...
myTake mi ml = do
	xs <- ml
	n <- mi
	if n > length xs then Nothing else return $ take n xs
...
lengthAndCount x (y:ys) = do
	counts <- get
	put (if x == y then counts+1 else counts)
...
count x = do
	...
	case lookup x al of
		Nothing -> put $ (x,1):al
		Just v -> put $ (x,v+1):(delete (x,v) al)
